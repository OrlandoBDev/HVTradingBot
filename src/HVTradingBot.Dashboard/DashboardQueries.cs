    ...
    CloseRequestStore closeRequests)
{
    ...
    public async Task<RiskStatusDto> GetRiskStatusAsync(CancellationToken cancellationToken)
    {
        ...
        var limits = new List<RiskLimitStatusDto>
        {
            ...
            new("Open positions", $"{positions.Count}", $"{risk.MaxOpenPositions} (+{risk.MaxExtraDerivedPositions} at score {risk.HighScoreOverrideMinScore}+)",
                positions.Count >= risk.MaxTotalPositions),
            new("Consecutive losses", $"{status.ConsecutiveLosses}", $"{risk.MaxConsecutiveLosses}", cooling),
            new("Derived open positions", $"{derivedOpen}", $"{risk.MaxDerivedOpenPositions} while Forex is open; any slot while closed", false),
